...
fetch(breeds_url)
    ...
    .then(data => {
        ...
        for(let i=0; i < breedsArray.length; i++){
            ...
            //loop to add each item from array to a new option
            option.innerText = breedsArray[i] // <option value='breed'>breed</option>
            select.appendChild(option) //creates additional item
        }
    })
...
const getDoggoImg = url => {
    fetch(url)  //going to the api url above
        .then(response => {
            return response.json() // get json message back
        })
        .then(data => {
            img.src = data.message //extrct message from json and attach img to tag as new source
        })
}
...
const doggoInfo = {
    ...
    assignLikes(array) {
        this.likes = this.yatesSuffle(this.likesList).slice(0, 2)
        document.getElementById('likes').innerHTML = `Likes: ${this.likes[0]}, ${this.likes[1]}`
    },
    assignDislikes() {
        this.dislikes = this.yatesSuffle(this.dislikesList).slice(0, 2)
    ...
    assignFact() {
        ...
        document.getElementById('fun-fact').innerText = `Fun Fact: ${this.fact}`
    },
    yatesSuffle(array) {
